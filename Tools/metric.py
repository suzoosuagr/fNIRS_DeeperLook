...
class Performance_Test_ensemble_binary(Metric):
    def __init__(self, joint=False, self_supervise=False):
        ...
        """
        super(Performance_Test_ensemble_binary, self).__init__()
        size = 2
    ...
    def __call__supervised(self, pred, truth):
        ...
        if self.joint:
            for p, t in zip(pred, truth):
                ...
                self.confusion_matrix['wml'][self.joint_label[int(t)][0]][self.locator(wml_p0)] += 1.0
                self.confusion_matrix['vpl'][self.joint_label[int(t)][1]][self.locator(vpl_p0)] += 1.0
    ...
    def __call__self_supervised(self, pred, truth):
        ...
        for i in range(B):
            ...
            self.confusion_matrix['wml'][self.joint_label[int(truth[0][i]//2)][0]][self.locator(wml_p0)] += 1.0
            self.confusion_matrix['vpl'][self.joint_label[int(truth[0][i]//2)][1]][self.locator(vpl_p0)] += 1.0
    ...
    def __call__(self, pred, truth):
        if self.self_supervise:
            self.__call__self_supervised(pred, truth)
        else:
...
class Performance_Test_ensemble_multi(Metric):
    ...
    def __call__(self, pred, truth, svm=False):
        if svm:
            self.__call_svm(pred, truth)
        elif self.self_supervise:
